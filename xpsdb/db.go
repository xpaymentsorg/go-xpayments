...
// Database errors.
var (
	ErrInvalidSegmentType = errors.New("xpsdb: Invalid segment type")
)
...
// migrate converts a source LevelDB database to the new xpsdb formatted database.
func (db *DB) migrate() error {
	...
	if _, err := os.Stat(db.Path); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("xpsdb.DB.migrate: %s", err)
	}

	// Ignore if path has already been migrated.
	if ok, err := IsDBDir(db.Path); err != nil {
		return fmt.Errorf("xpsdb.DB.migrate: cannot check xpsdb directory: %s", err)
	} else if ok {
	...
	if ok, err := IsLevelDBDir(db.Path); err != nil {
		return fmt.Errorf("xpsdb.DB.migrate: cannot check leveldb directory: %s", err)
	} else if !ok {
		return nil
	}

	log.Info("begin xpsdb migration", "path", db.Path)

	// Remove partial migration, if exists.
	tmpPath := db.Path + suffix
	if err := os.RemoveAll(tmpPath); err != nil {
		return fmt.Errorf("xpsdb.DB.migrate: cannot remove partial migration: %s", err)
	}
	...
	log.Info("xpsdb migration complete", "path", db.Path)
...
// IsDBDir returns true if path contains an xpsdb.DB.
...
func IsLevelDBDir(path string) (bool, error) {
	...
	if _, err := io.ReadFull(f, buf); err != nil {
		return false, fmt.Errorf("xpsdb.DB.IsLevelDBDir: cannot read CURRENT: %s", err)
	}
