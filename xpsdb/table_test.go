...
func TestTable_Put(t *testing.T) {
	dir := MustTempDir()
	tbl := xpsdb.NewTable("test", dir, &xpsdb.StaticPartitioner{Name: "data"})
...
func TestTable_Compact(t *testing.T) {
	t.Run("OK", func(t *testing.T) {
		...
		tbl := xpsdb.NewTable("test", dir, xpsdb.NewBlockNumberPartitioner(1000))
		...
		} else if _, ok := segments[0].(*xpsdb.FileSegment); !ok {
			t.Fatalf("expected file segment(0), got %T", segments[0])
		} else if _, ok := segments[1].(*xpsdb.LDBSegment); !ok {
			t.Fatalf("expected ldb segment(1), got %T", segments[1])
		} else if _, ok := segments[2].(*xpsdb.LDBSegment); !ok {
			t.Fatalf("expected ldb segment(1), got %T", segments[1])
		}
	...
	t.Run("MinCompactionAge", func(t *testing.T) {
		...
		if err := tbl.Compact(context.Background()); err != nil {
			t.Fatal(err)
		} else if _, ok := tbl.SegmentSlice()[0].(*xpsdb.LDBSegment); !ok {
		...
		if err := tbl.Compact(context.Background()); err != nil {
			t.Fatal(err)
		} else if _, ok := tbl.SegmentSlice()[0].(*xpsdb.FileSegment); !ok {
