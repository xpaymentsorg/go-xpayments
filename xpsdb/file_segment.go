...
var (
	ErrImmutableSegment            = errors.New("xpsdb: immutable segment")
	ErrSegmentTypeUnknown          = errors.New("xpsdb: segment type unknown")
	ErrFileSegmentChecksumMismatch = errors.New("xpsdb: file segment checksum mismatch")
)

const (
	// FileSegmentMagic is the magic number at the beginning of the file segment.
	FileSegmentMagic = "XPS1"
...
func (s *FileSegment) Open() error {
	...
	if len(data) < FileSegmentHeaderSize {
		s.Close()
		return errors.New("xpsdb: file header too short")
	} else if string(data[:len(FileSegmentMagic)]) != FileSegmentMagic {
		s.Close()
		return errors.New("xpsdb: invalid xpsdb file")
	}
	return nil
}
...
// FileSegmentEncoder represents a encoder for building a xpsdb.FileSegment.
...
func (enc *FileSegmentEncoder) Open() (err error) {
	if enc.f != nil {
		return errors.New("xpsdb: file already open")
	}
...
func (enc *FileSegmentEncoder) Flush() error {
	if enc.flushed {
		return errors.New("xpsdb: file index already flushed")
	}
	enc.flushed = true

	if err := enc.writeIndex(); err != nil {
		return fmt.Errorf("xpsdb: cannot write index: %s", err)
	} else if err := enc.writeChecksum(); err != nil {
		return fmt.Errorf("xpsdb: cannot write checksum: %s", err)
...
func (idx *fileSegmentEncoderIndex) insert(offset int64) error {
	...
	for {
		...
		if bytes.Equal(curr, key) {
			return errors.New("xpsdb: duplicate key written to file segment")
		}
