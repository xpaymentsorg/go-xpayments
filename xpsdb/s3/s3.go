...
var (
	uploadBytesMeter   = metrics.NewRegisteredMeter("xpsdb/s3/upload/bytes", nil)
	downloadBytesMeter = metrics.NewRegisteredMeter("xpsdb/s3/download/bytes", nil)
)
...
func ConfigureDB(db *xpsdb.DB, config xpsdb.Config) error {
...
func (c *Client) Open() (err error) {
	...
	if ok, err := c.client.BucketExists(c.Bucket); err != nil {
		...
		return fmt.Errorf("xpsdb/s3: bucket does not exist: %s", c.Bucket)
	}
	return nil
}
...
func (c *Client) tryFGetObject(ctx context.Context, key, path string) (err error) {
	...
	if err := xpsdb.VerifyFileSegment(tmpPath); err != nil {
		os.Remove(tmpPath)
		return err
	}
...
// Segment represents an xpsdb.FileSegment stored in S3.
type Segment struct {
	mu       sync.RWMutex
	muEnsure sync.Mutex // lock during check for file existence.

	client  *Client
	segment *xpsdb.FileSegment
...
func (s *Segment) ensureFileSegment(ctx context.Context) error {
	...
	s.segment = xpsdb.NewFileSegment(s.name, s.path)
...
func (s *Segment) Iterator() xpsdb.SegmentIterator {
...
var _ xpsdb.SegmentIterator = (*SegmentIterator)(nil)

// SegmentIterator represents a wrapper around xpsdb.SegmentIterator.
// Releases read lock on close.
type SegmentIterator struct {
	xpsdb.SegmentIterator
	segment *Segment
}
...
var _ xpsdb.SegmentOpener = (*SegmentOpener)(nil)
...
func (o *SegmentOpener) ListSegmentNames(path, table string) ([]string, error) {
	// Fetch local keys.
	localKeys, err := xpsdb.NewFileSegmentOpener().ListSegmentNames(path, table)
	if err != nil {
		return nil, err
	}
...
func (o *SegmentOpener) OpenSegment(table, name, path string) (xpsdb.Segment, error) {
	return NewSegment(o.Client, table, name, path), nil
}

// Ensure implementation fulfills interface.
var _ xpsdb.SegmentCompactor = (*SegmentCompactor)(nil)

// SegmentCompactor wraps xpsdb.FileSegmentCompactor and uploads to S3 after compaction.
...
func (c *SegmentCompactor) CompactSegment(ctx context.Context, table string, s *xpsdb.LDBSegment) (xpsdb.Segment, error) {
	fsc := xpsdb.NewFileSegmentCompactor()
...
func (c *SegmentCompactor) UncompactSegment(ctx context.Context, table string, s xpsdb.Segment) (*xpsdb.LDBSegment, error) {
	return xpsdb.NewFileSegmentCompactor().UncompactSegment(ctx, table, s)
}
