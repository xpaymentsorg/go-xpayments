...
func makeTestTrie() (*Database, *Trie, map[string][]byte) {
	...
	triedb := NewDatabase(xpsdb.NewMemDatabase())
...
func TestEmptySync(t *testing.T) {
	dbA := NewDatabase(xpsdb.NewMemDatabase())
	dbB := NewDatabase(xpsdb.NewMemDatabase())
	emptyA, _ := New(common.Hash{}, dbA)
	emptyB, _ := New(emptyRoot, dbB)

	for i, trie := range []*Trie{emptyA, emptyB} {
		if req := NewSync(trie.Hash(), xpsdb.NewMemDatabase(), nil).Missing(1); len(req) != 0 {
