...
type queue struct {
	...
	headerHead      common.Hash                    // [xps/62] Hash of the last queued header to verify order
	headerTaskPool  map[uint64]*types.Header       // [xps/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [xps/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [xps/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [xps/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [xps/62] Result cache accumulating the completed headers
	headerProced    int                            // [xps/62] Number of headers already processed from the results
	headerOffset    uint64                         // [xps/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [xps/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [xps/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [xps/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [xps/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [xps/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [xps/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [xps/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [xps/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [xps/63] Set of the completed receipt fetches
