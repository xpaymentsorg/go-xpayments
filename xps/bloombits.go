...
func (xpayments *XPS) startBloomHandlers() {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-xpayments.shutdownChan:
					return

				case request := <-xpayments.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(xpayments.chainDb, (section+1)*params.BloomBitsBlocks-1)
						compVector := rawdb.ReadBloomBits(xpayments.chainDb.GlobalTable(), task.Bit, section, head)
