...
FROM puppxps/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gpay --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gpay --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --netstats \'{{.Netstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PuppxpsNetworkID/{{.NetworkID}}/g' dist/js/xpswallet-master.js && \
	sed -i 's/PuppxpsNetwork/{{.Network}}/g'     dist/js/xpswallet-master.js && \
	sed -i 's/PuppxpsDenom/{{.Denom}}/g'         dist/js/xpswallet-master.js && \
	sed -i 's/PuppxpsHost/{{.Host}}/g'           dist/js/xpswallet-master.js && \
	sed -i 's/PuppxpsRPCPort/{{.RPCPort}}/g'     dist/js/xpswallet-master.js
