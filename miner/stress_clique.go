...
func main() {
	...
	for _, node := range nodes {
		...
		if err := node.Service(&xpayments); err != nil {
			panic(err)
		}
		if err := xpayments.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var xpayments *xps.XPS
		if err := nodes[index%len(nodes)].Service(&xpayments); err != nil {
			panic(err)
		}
		...
		if err := xpayments.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := xpayments.TxPool().Stats(); pend > 2048 {
