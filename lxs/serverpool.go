...
func (pool *serverPool) dial(entry *poolEntry, knownSelected bool) {
	...
	log.Debug("Dialing new peer", "lxsaddr", entry.id.String()+"@"+addr.strKey(), "set", len(entry.addr), "known", knownSelected)
	entry.dialed = addr
	go func() {
...
func (pool *serverPool) checkDialTimeout(entry *poolEntry) {
	...
	log.Debug("Dial timeout", "lxsaddr", entry.id.String()+"@"+entry.dialed.strKey())
	entry.state = psNotConnected
	if entry.knownSelected {
		pool.knownSelected--
	} else {
