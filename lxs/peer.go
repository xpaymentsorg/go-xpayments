...
// SendProofs sends a batch of legacy LXS/1 merkle proofs, corresponding to the ones requested.
...
// SendHeaderProofs sends a batch of legacy LXS/1 header proofs, corresponding to the ones requested.
	...
	case lpv1:
		...
		for i, req := range reqs {
			if req.Type != htCanonical || req.AuxReq != auxHeader || len(req.Key) != 8 {
				return fmt.Errorf("Request invalid in LXS/1 mode")
			}
...
func (p *peer) Handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, server *LxsServer) error {
	p.lock.Lock()
	defer p.lock.Unlock()
	...
	if server != nil {
		// until we have a proper peer connectivity API, allow LXS connection to other servers
...
// the Light xPayments sub-protocol.
type peerSet struct {
