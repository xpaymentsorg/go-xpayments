...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LxsOdr, db common.Database) (*ProtocolManager, error) {
	var (
	...
	if lightSync {
		...
		bloomIndexer := xps.NewBloomIndexer(db, params.BloomBitsBlocks)
	...
	if !lightSync {
		srv := &LxsServer{protocolManager: pm}
		pm.server = srv
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LxsOdr, db common.Database) *ProtocolManager {
