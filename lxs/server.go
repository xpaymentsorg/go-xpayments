...
func NewLxsServer(xps *xps.XPS, config *xps.Config) (*LxsServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(xps.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, xps.EventMux(), newPeerSet(), xps.BlockChain(), xps.TxPool(), xps.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}

	lxsTopics := make([]discv5.Topic, len(AdvertiseProtocolVersions))
	for i, pv := range AdvertiseProtocolVersions {
		lxsTopics[i] = lxsTopic(xps.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LxsServer{
		...
		quitSync:         quitSync,
		lxsTopics:        lxsTopics,
		chtIndexer:       light.NewChtIndexer(xps.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(xps.ChainDb(), false),
	}
	logger := log.New()

	chtV1SectionCount, _, _ := srv.chtIndexer.Sections() // indexer still uses LXS/1 4k section size for backwards server compatibility
	chtV2SectionCount := chtV1SectionCount / (light.CHTFrequencyClient / light.CHTFrequencyServer)
	if chtV2SectionCount != 0 {
		// convert to LXS/2 section
		chtLastSection := chtV2SectionCount - 1
		// convert last LXS/2 section index back to LXS/1 index for chtIndexer.SectionHead
	...
	srv.chtIndexer.Start(xps.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(xps.ChainDb())
	return srv, nil
}
...
func (s *LxsServer) Start(srvr *p2p.Server) {
	s.protocolManager.Start(s.config.LightPeers)
	if srvr.DiscV5 != nil {
		for _, topic := range s.lxsTopics {
			topic := topic
			go func() {
...
func (s *LxsServer) SetBloomBitsIndexer(bloomIndexer *core.ChainIndexer) {
	bloomIndexer.AddChildIndexer(s.bloomTrieIndexer)
}

// Stop stops the LXS service
func (s *LxsServer) Stop() {
	if err := s.chtIndexer.Close(); err != nil {
		log.Error("Cannot close lxs chain indexer", "err", err)
	}
...
func (s *requestCostStats) store() {
	...
	if data, err := rlp.EncodeToBytes(statsRlp); err == nil {
		if err := s.db.GlobalTable().Put(rcStatsKey, data); err != nil {
			log.Error("Cannot put lxs req cost stats", "err", err)
		}
	}
}
...
func (pm *ProtocolManager) blockLoop() {
	...
	pm.blockchain.SubscribeChainHeadEvent(headCh, "lxs.ProtocolManger-blockLoop")
	go func() {
