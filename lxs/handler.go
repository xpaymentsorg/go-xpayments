...
const (
	...
	xpsVersion = 63 // equivalent xps version for the downloader
...
// NewProtocolManager returns a new xpayments sub protocol manager. The xPayments sub protocol manages peers capable
// with the xpayments network.
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *core.InterfaceFeed, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb common.Database, odr *LxsOdr, txrelay *LxsTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light xPayments protocol")
	...
	log.Info("Light xPayments protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of a lxs peer. When
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case GetProofsV2Msg:
		p.Log().Trace("Received lxs/2 proofs request")
		// Decode the retrieval message
		var req struct {
			ReqID uint64
			Reqs  []ProofReq
...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // xPayments network ID (150=Mainnet, 151=Beryllium)
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	if err := pm.downloader.RegisterLightPeer(p.id, xpsVersion, pc); err != nil {
